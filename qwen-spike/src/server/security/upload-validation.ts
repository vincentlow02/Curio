...
export function validateUpload(file: File, maxBytes: number): { extension: string } {
  ...
  if (!extension) throw new Error("图片格式不支持，仅支持 JPG、PNG、WEBP。");
  if (file.size <= 0) throw new Error("上传的图片为空。");
  if (file.size > maxBytes) throw new Error(`图片超过 ${(maxBytes / 1024 / 1024).toFixed(0)} MB 限制。`);
  return { extension };
}
