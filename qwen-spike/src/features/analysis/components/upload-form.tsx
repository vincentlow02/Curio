...
export function UploadForm(): React.ReactElement {
  ...
  function select(next: File | null): void {
    setError(null);
    if (!next) return;
    if (!ACCEPTED.has(next.type)) { setError("图片格式不支持，请选择 JPG、PNG 或 WEBP。"); return; }
    if (next.size > MAX_BYTES) { setError("图片超过 10 MB，请压缩后重新上传。"); return; }
  ...
  async function submit(): Promise<void> {
    if (!demoCode.trim()) { setError("请输入演示访问码。"); return; }
    if (!file) { setError("请先选择一张收藏品图片。"); return; }
    ...
    try {
      ...
      if (!response.ok || !body.sessionId) throw new Error(body.error ?? "无法创建分析任务。");
  ...
  return <section className="hero-grid">
    ...
    <div className="upload-card">
      ...
      <div className={`drop-zone ${dragging ? "is-dragging" : ""} ${preview ? "has-preview" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop} onClick={() => input.current?.click()} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter") input.current?.click(); }}>
        ...
        {preview ? <><img src={preview} alt="待分析收藏品预览" /><div className="preview-meta"><b>{file?.name}</b><span>Click or drop to replace</span></div></> : <><span className="upload-icon">↥</span><b>Drop your collectible photo here</b><span>or click to browse · JPG, PNG, WEBP · max 10 MB</span></>}
      </div>
      ...
      <p className="privacy-note">Your image is deleted immediately after identification.</p>
    </div>
  </section>;
}
